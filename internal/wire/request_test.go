...
// parse checks a request the way the generator does, so that a test builds a
// request from the same thing a run does.
func parse(t *testing.T, name, src string) *request.Request {
	t.Helper()
...
// values reads the parameters of a request as a caller would give them on a
// command line.
func values(t *testing.T, q *request.Request, pairs map[string]string) map[string]Value {
	t.Helper()
...
func build(t *testing.T, q *request.Request, vals map[string]Value, accounts Accounts) string {
	t.Helper()
...
// TestBuild checks a request carrying every kind of value a request holds: a
...
func TestBuild(t *testing.T) {
	...
	for _, want := range []string{
		...
		// The account id the request left out is filled in from the session.
		`"accountId":"a1"`,
	} {
...
// TestBuildKeepsTheAccountTheQueryStates checks that a request naming the account
...
func TestBuildKeepsTheAccountTheQueryStates(t *testing.T) {
	...
	accounts := func(string) (jmapc.ID, error) {
		t.Error("the session was asked for an account the request states")
		return "", nil
	}
	if got := build(t, q, nil, accounts); !strings.Contains(got, `"accountId":"a2"`) {
		t.Errorf("the request does not carry the account the request names:\n%s", got)
	}
}

// TestBuildWithoutASession checks that a request leaving the account to the
...
// TestBuildLeavesOutAnOptionalParameter checks the argument a request lets the
