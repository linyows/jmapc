...
// looks at.
func param(t *testing.T, typ string) *request.Param {
	t.Helper()
	return &request.Param{Name: "p", Type: spec.MustParseType(typ)}
}
...
func TestParseValue(t *testing.T) {
	cases := []struct{ typ, text, want string }{
		...
		// An accepted value the request leaves nullable is still a value: a
		// caller who means null writes it in the request.
