// Package wire turns a checked request into the JMAP request it stands for,
// with the parameters filled in. The generator writes that request as source;
// here the same request is built at run time, which is what lets a request be
...
// whose accountId the request leaves out.
type Accounts func(capability string) (jmapc.ID, error)

// Build returns the request a request stands for, with values filling in the
// parameters it left open and accounts filling in the account ids it did not
// state.
func Build(s *spec.Spec, q *request.Request, values map[string]Value, accounts Accounts, createdIDs map[jmapc.ID]jmapc.ID) (*jmapc.Request, error) {
	...
	for _, c := range q.Calls {
		...
		if capability, ok := c.AccountIDCapability(s); ok {
			...
			args[request.AccountIDArgument] = id
		}
...
// supplied that the request does not have, both at once: a caller who mistyped
...
func CheckValues(q *request.Request, values map[string]Value) error {
...
func hint(name string, params []*request.Param) string {
...
func (b *builder) accountID(capability string) (jmapc.ID, error) {
	...
	if b.accounts == nil {
		return "", fmt.Errorf("the request leaves accountId to the session's primary account for %s, and there is no session to ask", capability)
	}
...
// JSON it already is, which keeps the request the shape the request wrote.
func (b *builder) node(n request.Node) (any, error) {
	switch v := n.(type) {
