...
// Error is one problem found in a request file, located by the path through the
// JSON document rather than by line, so that the message points at the argument
// that is wrong.
type Error struct {
	// File is the request file the problem was found in.
	File string
	// Where is the path through the request document, such as
...
// ErrorList is every problem found in one request file. Parsing reports all of
// them at once, so that a request can be fixed in one pass.
