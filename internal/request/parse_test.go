...
// parse checks src as the request named by path and fails the test if it does not
// parse cleanly.
func parse(t *testing.T, path, src string) *Request {
	t.Helper()
...
func TestParse(t *testing.T) {
	...
	// The field is the name the request gave the call, not the method it
...
func TestParseErrors(t *testing.T) {
	tests := []struct {
		...
		src:  `{"methodCalls": []}`,
		want: `the request makes no method calls`,
	}}
...
// TestSubmissionQuery covers a request that spans two capabilities, checking that
// both are declared.
...
// else is a request that would fail against a conforming server.
...
// TestContactsQuery covers a request against JMAP for Contacts, whose cards are
...
// TestContactsChecks covers the mistakes a contacts request can make.
func TestContactsChecks(t *testing.T) {
	tests := []struct {
		...
		name: "address book has no request",
...
// TestCalendarsQuery covers a request against JMAP for Calendars, whose events
...
// server, which is free to ignore a value it does not recognise, so the request
...
// TestPrincipalsChecks covers the mistakes a sharing request can make.
...
func TestSMIMECapabilityIsTracked(t *testing.T) {
	// Derived from the properties, when the request says nothing.
	...
	// Reported, when the request declares a set that does not cover it.
	got := parseErr(t, `{
	  "using": ["core", "mail"],
	...
	// A request that touches none of them needs nothing extra.
...
// make every annotated request fail.
...
// TestJSONCommentsAreNotAccepted checks that a request file is plain JSON. It was
...
func TestPushSubscription(t *testing.T) {
	...
	for _, p := range q.Params {
		if p.Name == "accountId" {
			t.Error("the request has an accountId parameter, which these methods do not take")
		}
	}
...
// TestCreatedIDsCarry covers a request that takes the creation ids of an earlier
...
func TestCreatedIDsCarry(t *testing.T) {
	...
	if !q.CreatedIDs {
		t.Error("the request does not carry creation ids")
	}

	// The ids belong to the request rather than to any one call, so there is
	// nowhere to put them in a request that returns a single response.
	got := parseErr(t, `{
	  "_createdIds": true,
	  "_returns": "c0",
	...
	// A request that says nothing about them carries none.
	plain := parse(t, "Plain"+Extension, `{"methodCalls": [["Mailbox/get", {}, "c0"]]}`)
	if plain.CreatedIDs {
		t.Error("a request that did not ask for creation ids carries them")
	}
}

// TestRequestMayNameItsSchema checks that a request file can point at the JSON
...
func TestRequestMayNameItsSchema(t *testing.T) {
...
// syncEmails is a request a watch can be built on: it reports what changed since
...
// TestWatchesRejects covers the requests a loop cannot be built on, each of
...
// TestPages checks what a request says when it asks to be paged: which call the
...
// TestPagesRejects covers the requests a walk cannot be built on, each of which
...
func TestPagesRejects(t *testing.T) {
	cases := []struct{ name, src, want string }{
		...
		{
			"a position the request states itself",
			`{"_pages": "search", "methodCalls": [["Email/query", {"position": 0}, "search"]]}`,
			"has to be a parameter",
		},
		{
			"a position the request leaves out",
...
// TestShapeIgnoresTheNames checks what makes two requests one request: the names
// given to parameters and to calls are the request's own business, and the
...
func TestShapeIgnoresTheNames(t *testing.T) {
	...
	if one.Shape != two.Shape {
		t.Errorf("two names for one request have different shapes:\n%s\n%s", one.Shape, two.Shape)
	}
}

// TestShapeKeepsWhatIsAsked checks the other half: a request that asks the server
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if base.Shape == other.Shape {
				t.Errorf("a request asking for something else has the same shape:\n%s", other.Shape)
			}
		})
	}
}

// TestShapeCountsHowManyParametersThereAre checks that a request taking one
// parameter where another takes two is not the same request, since the caller
...
func TestShapeCountsHowManyParametersThereAre(t *testing.T) {
	...
	if two.Shape == one.Shape {
		t.Errorf("a request taking one parameter has the shape of one taking two:\n%s", one.Shape)
	}
}

// TestCreationIDsAreCollected checks what a request says it creates. The names
// are the request's to invent, and a caller reads the created records back under
...
// TestCreationIDsLeftToTheCallerAreNotNamed checks a creation id the request
...
func TestCallFieldsAreNamedAfterTheCall(t *testing.T) {
	...
	// The same request with a call inserted ahead of them: the two keep their
