...
// that says the caller may leave the argument out, since a request that lets one
// be left out is not the same request as one that does not.
var shapeParamPattern = regexp.MustCompile(`\{\{\s*([A-Za-z_][A-Za-z0-9_]*)\s*(\?)?\s*\}\}`)

// shapeOf returns the request with its names taken off: the parameters numbered
// by where they first appear, the call ids numbered by position, and the
// documentation dropped. Two requests with the same shape are one request written
// twice, whatever the two call things.
//
// It works from the source rather than from the checked request, because what is
...
// shaper carries the numbering while a request is walked.
type shaper struct {
...
// request chooses reaches neither the generated names nor the meaning of the
// request.
