...
今このリクエストを受けられないサーバは、二つのうちどちらかでそう言います。
429 と 503 で、どちらも「何も起きなかった」を意味します。
`WithRetry` は、それを見たらリクエストを送り直します。

```go
c := jmapc.New(url, jmapc.WithBearerToken(token), jmapc.WithRetry(3))
```

3 は、最初の一回を含めて何回送るかです。
待ち時間は、サーバが `Retry-After` で求めた長さです。
特に何も求められなければ、0.2 秒から 30 秒へ倍々に待ちます。
1 分より長く待てと言うサーバは、リクエストをそれまでメモリに座らせておけと言っているのではなく、呼び出し側にあとで出直せと言っています。
ですからその場合は、待たずに拒否を返します。

これ以外は送り直しません。
行きか帰りで失敗したリクエストは、実行されてしまっているかもしれず、二度送られた `/set` は二つ作るからです。
送り直しても安全なリクエストしかしないクライアントは、`WithRetryPolicy` でそう言えます。待ち時間を差し替えるのもそこです。

そもそも拒否に歩いていかないことのほうが大事で、そちらにはオプションは要りません。
セッションは、サーバが一度に受けるリクエスト数とアップロード数を告げています。
クライアントはその両方に自分を従わせ、上限を超える一つは、送って追い返されるのではなく空きを待ちます。

## テスト

生成されたクライアントの周りに書いたコードをテストするということは、複数のメソッド呼び出しを載せたリクエストに答えるということです。
しかもその一部は、他の呼び出しの結果を参照しています。
一つのテストのために手で書いたスタブは、そこを無視してサーバに似ることをやめるか、さもなければこれに育ちます。
なので、これを用意しました。
