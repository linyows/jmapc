...
生成されたコードはリクエストの形でしかありません。
呼び出しと答えのあいだで起きることは、このパッケージが提供するクライアントの担当です。
JMAP が報告した層で失敗を報告し、セッションを保ち、変わったとサーバが言えば取り直し、一度に送るには大きすぎる `/get` を分割し、期限の切れるトークンを取り直し、やり直す価値のあるものをやり直し、何をしたかを報告します。
どれも既定の挙動で、オプションで変えられます。
