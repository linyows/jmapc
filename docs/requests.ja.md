...
形そのものに名前を付ければ、6 本が 1 つの型を返します。

集合はリクエストの隣の `properties.json` に書きます。
これは JMAP のリクエストではないので `.jmap.json` を名乗りません。
...
| メンバ | |
|---|---|
| `type` | プロパティを選ぶ対象のデータ型です。必須ですが、`extends` が型を決めている場合は省略できます。 |
...
リクエストは、プロパティの列挙の代わりに集合の名前を書きます。
...
サーバに届くのは集合が持つプロパティの列挙です。
集合はコードを生成するときに解決され、`{{name}}` は実行時に呼び出し側が埋めます。書き方が違うのはそのためです。

生成される型は集合の名前を取ります。
`ListInboxEmailsFetchEmail` ではなく `EmailSummary` です。
その集合を求めるリクエストはすべてこの 1 つの型を返すので、この型に対して書いた関数が、どのリクエストの結果でも受け取れます。
プロパティの列挙も 1 箇所になります。
アプリケーションが読むプロパティを 1 つ増やすとき、編集はリクエストごとではなく 1 回で済み、どれか 1 本だけ古いまま少なく要求し続けることが起きません。

他の集合を継承した集合は、継承元を埋め込みます。
継承元に対して書いた関数が、二つの構造体の間で値を詰め替えることなく、継承先のレコードを受け取れます。
...
Rust は継承元を継承先の構造体に flatten し、TypeScript はインタフェースを継承します。どちらでも同じことが成り立ちます。

集合の名前は、呼び出し側が書く型の名前です。
そのためリクエストが同じ名前を取ることはできません。
`EmailSummary` という集合の隣に `EmailSummary.jmap.json` があれば、黙って改名されるのではなくエラーとして報告されます。

集合を求める呼び出しは、レコードについてそれ以上の絞り込みをしません。
...
同じ名前付き集合を求める呼び出しどうしは、パッケージのどこにあってもその集合の型を共有します。
同じプロパティを書き下しただけの呼び出しはここに含まれません。集合が指す名前は作者が選んだものだからです。
